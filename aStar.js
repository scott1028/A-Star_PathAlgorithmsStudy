...
// 計算路徑的遞迴
    var calPaths=function(pos){
        var updateLastPos = function(originNewPos, close){
            var fixedNewPos = {
                lastPos: null,
                value: null
            }
            close.every(function(row, index){
                if(row == originNewPos) return true;
                if(fixedNewPos.value === null){
                    fixedNewPos.value = Math.abs(originNewPos.x - row.x) + Math.abs(originNewPos.y - row.y);
                    fixedNewPos.lastPos = row;
                }
                else{
                    if(fixedNewPos.value > Math.abs(originNewPos.x - row.x) + Math.abs(originNewPos.y - row.y)){
                        fixedNewPos.value = Math.abs(originNewPos.x - row.x) + Math.abs(originNewPos.y - row.y);
                        fixedNewPos.lastPos = row;
                    };
                }
